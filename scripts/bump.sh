...
# then commit and release (see README "Develop & Release").
set -euo pipefail
cd "$(dirname "$0")/.."
...
# Resolve the new version: either an explicit X.Y.Z, or a semver increment.
...
# Guard: refuse to set a version that already has a CHANGELOG section.
if grep -qE "^## v${NEW//./\\.}$" CHANGELOG.md 2>/dev/null; then
  echo "CHANGELOG.md already has a section for v$NEW — aborting." >&2
  exit 1
fi

# Rewrite manifest.json version, preserving 2-space indentation.
...
# Prepend a new "## vNEW" section just under the top "# Changelog" heading.
NEW="$NEW" node -e '
  var fs = require("fs");
  var p = "CHANGELOG.md";
  ...
  // Find the "# Changelog" title line; insert after it (and its trailing blank).
  var idx = -1;
